...
POSSIBLE_ANSWERS = ["No", "Maybe", "Yes"]
CATEGORY_NAMES = ["General", "Education", "Health", "Protection - MHPSS - CFS",
                  "WASH"]
SUBCATEGORY_NAMES = ["General", "Refugees/IDPs/Migrants", "4WS",
                     "Coordination and Collaboration",
                     "Cultural and Social Factors",
                     "Demographics and Community Structure",
                     "Economic Factors", "Infrastracture and Services",
                     "Psychosocial", "Health Information System",
                     "Immunizations", "Mortality", "Nutrition", "Problems",
                     "Services Quality", "Community", "vulnerable populations",
                     "Drinage", "Hygiene", "Sanitation Excreta disposal",
                     "Solid waste disposal", "Vector-borne disease",
                     "Water supply and access"]

SUBCATEGORY_GRADES = {"General": 0, "Refugees/IDPs/Migrants": 0, "4WS": 0,
                      "Coordination and Collaboration": 0,
                      "Cultural and Social Factors": 0,
                      "Demographics and Community Structure": 0,
                      "Economic Factors": 0, "Infrastracture and Services": 0,
                      "Psychosocial": 0, "Health Information System": 0,
                      "Immunizations": 0, "Mortality": 0, "Nutrition": 0,
                      "Problems": 0, "Services Quality": 0, "Community": 0,
                      "vulnerable populations": 0, "Drinage": 0, "Hygiene": 0,
                      "Sanitation Excreta disposal": 0,
                      "Solid waste disposal": 0, "Vector-borne disease": 0,
                      "Water supply and access": 0}

sub_and_cat_dict = {"General": "Education",
                    "Refugees/IDPs/Migrants": "Education", "4WS": "General",
                    "Coordination and Collaboration": "General",
                    "Cultural and Social Factors": "General",
                    "Demographics and Community Structure": "General",
                    "Economic Factors": "General",
                    "Infrastracture and Services": "General",
                    "Psychosocial": "General",
                    "Health Information System": "Health",
                    "Immunizations": "Health", "Mortality": "Health",
                    "Nutrition": "Health", "Problems": "Health",
                    "Services Quality": "Health",
                    "Community": "Protection - MHPSS - CFS",
                    "vulnerable populations": "Protection - MHPSS - CFS",
                    "Drinage": "WASH", "Hygiene": "WASH",
                    "Sanitation Excreta disposal": "WASH",
                    "Solid waste disposal": "WASH",
                    "Vector-borne disease": "WASH",
                    "Water supply and access": "WASH"}

CAT_TO_SUBCAT = {"Education": ["General", "Refugees/IDPs/Migrants"],
                 "General": ["4 Ws", "Coordination and Collaboration",
                             "Cultural and Social Factors",
                             "Demographics and Community Structure",
                             "Economic Factors",
                             "Infrastracture and Services",
                             "Psychosocial"],
                 "Health": ["Health Information System", "Immunizations",
                            "Moratlity", "Nutrition", "Problems",
                            "Services Quality"],
                 "Protection - MHPSS - CFS": ["Community",
                                              "vulnerable populations"],
                 "WASH": ["Drinage", "Hygiene",
...
},
    {
        ...
        "question": {
            ...
            "options": ["Within the last 6 months", "Within the last year",
                        "Within the last 2 years", "Within the last 5 years"],
            "dependency": None
        },
        "rna": {
            "id": "6sah2013",
...
class PDF(FPDF):
    ...
    def general_details(self, json):
        item = json[0]
        last_date = item['rna']['lastupdatedate']
        creation_date = item['rna']['creationdate']
...
def generate_txt(json):
    ...
    for category in categories:
        with open(f'{category}.txt', 'w') as f:
            for subcat in CAT_TO_SUBCAT[category]:
                f.write(f"\n{subcat}-\n")
                for item in json:
                    if item['category']['name'] == category and item['subcategory']['name'] == subcat:
                        ...
                        for answer in item['answer']['value']:
                            if answer != item['answer']['value'][-1]:
                                ...
                                answer_string += f'{answer}\n'
                        f.write(f"{item['question']['id']}) {item['question']['question']} - {answer_string}")
        with open(f'{category}_images.txt', 'w') as file:
            for item in json:
                if item['category']['name'] == category:
                    images_string = ''
                    if item['answer']['photo'] is None:
                        continue
                    else:
                        for answer in item['answer']['photo']:
...
def run_RNA(list):
    ...
    for item in list:
        if str(item["rna"]["id"]) not in rna_dict.keys():
            rna_dict[item["rna"]["id"]] = SUBCATEGORY_GRADES
    return rna_dict
...
def rna_measure(json):
    ...
    rna_final_grades = {}
    for rna in rna_dict:
        for cat in CAT_TO_SUBCAT:
            subcat_grades = []
            for subcat in rna_dict[rna]:
                if subcat in CAT_TO_SUBCAT[cat]:
...
def create_percent_chart(json):
    ...
    fig.show(width=600, height=200)
    fig.write_image("progress.png")
...
def explain_percentages(perc):
    if 0 <= perc < 25:
        return f'Only {perc}% of the questionnaire was completed, the RNA is not entirely trustworthy.'
    elif 25 <= perc < 50:
        return f'Only {perc}% of the questionnaire was completed, some of the insights may not be precised.'
    elif 50 <= perc < 75:
        return f'Only {perc}% of the questionnaire was completed, please take that into consideration.'
    elif 75 <= perc <= 100:
...
def create_PDF_File():
    ...
    with open('insights.txt', 'w') as insights_file:
        ...
        insights_file.write(percent_txt)
